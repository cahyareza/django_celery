...
# Syncronous task execution
def test_sync():
    result = t1.apply_async(args=[5, 10], kwargs={"message": "The sum is"})
    print("Task executed synchronously:", result.get())

def test_async():
    result = t1.apply_async(args=[5, 10], kwargs={"message": "The sum is"})
    print("Task executed asynchronously, check the worker logs for output.", result.task_id)
